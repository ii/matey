...
Exit 0: writable (marker echoed). Exit 2: connects and streams output but
typed input never lands — a read-only server or a wedged connection. Exit 1:
can't connect at all. It exists because a single wedged websocket in a
client is indistinguishable from "read-only", and once got a healthy stack
mis-diagnosed as a ttyc/ttyd protocol incompatibility; the probe answers
which side is broken in ~3 seconds.
* coder
#+begin_src shell
